/***************************************************************************
 * Portland Doors                                                          *
 * unref2.c:  Test driver for the DOOR_UNREF and DOOR_IS_UNREF flags.      *
 *                                                                         *
 *            Correct output is as follows:                                *
 *                                                                         *
 *            There should be exactly one "Unreferenced invocation         *
 *            received" message, which follows this line.                  *
 *            Unreferenced invocation received and DOOR_IS_UNREF is        *
 *            properly set.                                                *
 *            There should be no such message following this line.         *
 *                                                                         *
...
static void client_proc(void)
{
	...
	printf("There should be exactly one \"Unreferenced invocation "
	       "received\" message, which follows this line.\n"
	      );
	fflush(stdout);
...
/* Give the server time to detect that we have closed the descriptor. */
	...
	printf("There should be no such message following this line.\n");
	fflush(stdout);
